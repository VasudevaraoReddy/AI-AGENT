    ...
    try {
        ...
        try {
            ...
            try {
                ...
                console.error('Failed to parse service determination response:', secondError);
...
const serviceConfigTool = tool(
        ...
        try {
            ...
            if (!message) {
                return JSON.stringify({
                    response: "Please provide a message describing what you want to provision.",
                    tool_response: {
                        status: "error",
            ...
            const exampleValues = await generateExampleValues(serviceConfig.data[0], csp);
            const enhancedService = {
                ...serviceConfig.data[0],
                requiredFields: serviceConfig.data[0].requiredFields.map(field => ({
                    ...field,
                    exampleValue: exampleValues.fields.find(ef => ef.fieldId === field.fieldId)?.exampleValue || '',
                    explanation: exampleValues.fields.find(ef => ef.fieldId === field.fieldId)?.explanation || ''
                }))
            };
    ...
    },
    {
        ...
        description: 'Use this tool to get cloud service configurations based on user requests.',
        schema: z.object({
            csp: z.string().describe('The cloud service provider (AWS, Azure, GCP, or Oracle)'),
            message: z.string().describe('The user message describing the service they want to provision'),
        }),
    },
);
